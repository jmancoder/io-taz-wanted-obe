...
def read_bmp(input_path: Path) -> Texture:
    ...
    match format_id:
        ...
        case 3:
            ...
            g = (argb >> 5) & 0x1F
            b = argb & 0x1F
            pixels = np.stack((r, g, b, a), axis=2).astype(np.float32)
            pixels[:, :, :3] /= 31.0
            pixels[:, :, 3] /= 1.0
        ...
        case 7:
            ...
            pixels = palette[indices].reshape(height, width, -1)
            pixels = pixels[::-1].ravel()
        case 9:
            # ARGB4444
            argb = np.frombuffer(
            ...
            g = (argb >> 4) & 0xF
            b = argb & 0xF
            pixels = np.stack((r, g, b, a), axis=2).astype(np.float32) / 15.0
            pixels = pixels[::-1].ravel()
        case _:
            logging.error(
